      ...
      $(document).ready(function () {
          ...
          var customActionsHTML = function (params) {
              if (!superuserRoleIds.includes(params.data.roles[0].id)) {
                  ...
                  // var path = {{url('/users/edit')}};
                  @if(in_array('administrator:update',session('permissions')))
                  var editIconHTML = "<a href='" + "{{url('/users/edit')}}" + '/' + params.data.id + "'><i class='users-edit-icon feather icon-edit-1 mr-50'></i></a>";
          ...
          if (document.getElementById("users-table")) {
              ...
              $.ajax({
                  type: "GET",
                  url: "{{env('TODO_LIST_API_' . env('TODO_LIST_API_MODE') . '_ENDPOINT')}}" + "/role/administrator/list/" + (window.location.pathname.split("/").pop()),
                  headers: {
                      'Authorization': 'Bearer ' + JSON.parse(localStorage.getItem('user'))['access_token']
                  },
