            ...
            <ul class="nav navbar-nav">
              <li class="nav-item nav-search"><a class="nav-link nav-link-search"><i
                ...
                <div class="search-input">
                  ...
                  <div id="nav-endpoint" style="display: none;">{{env('TODO_LIST_API_' . env('TODO_LIST_API_MODE') . '_ENDPOINT')}}</div>
                </div>
              </li>
            </ul>
          </div>
...
<!--             <li class="dropdown dropdown-notification nav-item"><a class="nav-link nav-link-label" href="#"
            ...
            <li class="dropdown dropdown-user nav-item"><a class="dropdown-toggle nav-link dropdown-user-link" href="#"
                ...
                <div class="user-nav d-sm-flex d-none"><span class="user-name text-bold-600"></span><span class="user-role"></span></div><span><img class="round"
                    src="{{asset('images/logo/favicon.png') }}" alt="avatar" height="40"
                    width="40" /></span>
              </a>
              <div class="dropdown-menu dropdown-menu-right">
                @if(in_array('administrator:self:update',session('permissions')))
                <a class="dropdown-item" href="{{ url('/users/edit') }}">
                  <i class="feather icon-edit-1"></i> Update Self Profile
                </a>
                @endif
                ...
                <a class="dropdown-item" href="{{ url('/password/update') }}">
                  <i class="feather icon-edit"></i> Update Password
                </a>
                @endif

                <div class="dropdown-divider"></div>
                <a class="dropdown-item" onclick="logout()" href="{{ url('/logout') }}">
...
<script>
  function logout () {
      $.ajax({
        async: false,
        type: "GET",
        url: "{{env('TODO_LIST_API_' . env('TODO_LIST_API_MODE') . '_ENDPOINT')}}" + "/administrator/logout",
        headers: {
